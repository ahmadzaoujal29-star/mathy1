...
# (مُعدَّل لقراءة المفتاح من Streamlit Secrets)
client = None
try:
    ...
    st.error("خطأ في إعدادات Secrets: لم يتم العثور على 'google_api_key'. يرجى التأكد من إضافته في لوحة تحكم الاستضافة.")
    client = None
except APIError as e:
    st.error(f"خطأ في تهيئة Google Gemini API. الرجاء التأكد من صحة المفتاح. الخطأ: {e}")
    client = None
...
def get_gemini_response(prompt, image=None):
    ...
    """
    if not client:
        return "تعذر الاتصال بخدمة Gemini. يرجى مراجعة إعدادات API."
    ...
    try:
        ...
        return f"حدث خطأ في واجهة API: {e}. (قد يكون بسبب مشكلة في الإدخال أو الاتصال)."
    except Exception as e:
        return f"حدث خطأ غير متوقع أثناء الاتصال بالنموذج: {e}"
...
st.sidebar.markdown("---")
st.sidebar.markdown("صُمم خصيصاً لمشاريع اليوتيوب والمشاريع الإلكترونية. بالتوفيق في دراستك للعلوم الرياضية!")
